...
function navigate(page) {
  ...
  const titles = { dashboard:'Dashboard', shifts:'Turnos', invoices:'Facturas', profile:'Perfil' };
...
async function loadDashboard() {
  ...
  if (shiftsRes && shiftsRes.ok) {
    ...
    if (!items.length) {
      recentEl.innerHTML = `<div class="empty-state"><i class="bi bi-clock"></i><p>Sin turnos aún</p></div>`;
    } else {
  ...
  if (invRes && invRes.ok) {
    ...
    if (!items.length) {
      invEl.innerHTML = `<div class="empty-state"><i class="bi bi-receipt"></i><p>Sin facturas aún</p></div>`;
    } else {
...
async function loadShifts() {
  ...
  if (!res || !res.ok) {
    tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-4">Error al cargar turnos</td></tr>`;
    return;
  }
  ...
  document.getElementById('shifts-summary-bar').innerHTML = `
    <span class="summary-pill"><i class="bi bi-list-ul"></i> <strong>${items.length}</strong> turnos</span>
  ...
  if (!items.length) {
    tbody.innerHTML = `<tr><td colspan="7" class="text-center py-5 text-muted">Sin turnos para este filtro</td></tr>`;
    return;
  }
    ...
    <tr>
      ...
      <td class="text-center">
        <div class="d-flex gap-1 justify-content-center">
          ${s.status === 'pending' ? `
            <button class="btn btn-sm sit-btn-ghost py-0 px-2" onclick="editShift(${s.id})" title="Editar">
              <i class="bi bi-pencil"></i>
            </button>
            <button class="btn btn-sm sit-btn-ghost py-0 px-2 text-danger" onclick="deleteShift(${s.id})" title="Eliminar">
...
function openShiftModal(shift = null) {
  document.getElementById('shift-modal-title').textContent = shift ? 'Editar turno' : 'Nuevo turno';
...
async function saveShift(e) {
  ...
  setLoading(btn, false);
  if (res && res.ok) {
    showToast(id ? 'Turno actualizado' : 'Turno guardado', 'success');
    shiftModal.hide();
    loadShifts();
    if (currentPage === 'dashboard') loadDashboard();
  } else if (res) {
...
async function loadInvoices() {
  ...
  if (!res || !res.ok) {
    container.innerHTML = `<div class="col-12 text-danger text-center py-4">Error al cargar facturas</div>`;
    return;
  }
    ...
    container.innerHTML = `
      <div class="col-12">
        <div class="empty-state py-5">
          ...
          <p class="mt-2">No hay facturas. Genera una desde la vista de Turnos.</p>
        </div>
      </div>`;
    return;
  }
    ...
    <div class="col-md-6 col-xl-4">
      <div class="invoice-card">
        ...
        <div class="text-muted mb-3" style="font-size:12px">
          ${inv.client_name || '—'} · ${inv.shifts?.length || 0} turno(s)
        </div>
        <div class="d-flex gap-2 flex-wrap">
          ...
          <button class="btn sit-btn-ghost btn-sm" onclick="downloadExcel(${inv.id})" title="Descargar Excel">
...
async function deleteInvoice(id, isDraft) {
  if (!isDraft) {
    if (!confirm('¿Anular esta factura? Los turnos volverán a estado pendiente.')) return;
    const voidRes = await api(`/invoices/${id}/status/`, { method: 'PATCH', body: { status: 'void' } });
    if (!voidRes || !voidRes.ok) { showToast('Error al anular', 'danger'); return; }
  } else {
...
async function downloadExcel(id) {
  ...
  if (!res.ok) { showToast('Error al descargar Excel', 'danger'); return; }
...
function labelStatus(s) {
  return { pending:'Pendiente', invoiced:'Facturado', paid:'Pagado' }[s] || s;
}
